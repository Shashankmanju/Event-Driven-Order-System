...
User creates order → Sends request to Order Service
Order Service checks stock → Calls Product Service REST API (SYNCHRONOUS)
Product Service checks database → Returns availability (available/not available)
Order Service filters items → Only keeps available products
Order Service saves order → Stores in database with status "ORDER_PLACED"
Order Service publishes event → Sends to Kafka (ASYNCHRONOUS)
User gets response → "Order placed successfully"
Product Service listens to Kafka → Automatically triggered
Product Service reduces stock → Updates database
Done! → Order placed, stock reduced
⚡ Key Insights
...
# Event-Driven-Order-System # Event-Driven-Order-System
